...
describe(`${GetAdminNoticesRequestDto.name} Test`, () => {
  let dto: GetAdminNoticesRequestDto;

  beforeEach(() => {
    dto = new GetAdminNoticesRequestDto({
      page: 1,
      limit: 10,
  ...
  it('page와 limit이 입력되지 않을 경우 기본값이 적용되어 유효성 검사에 성공한다.', async () => {
    // given
    const defaultDto = new GetAdminNoticesRequestDto({});
