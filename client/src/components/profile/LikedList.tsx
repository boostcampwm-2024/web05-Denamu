...
export const LikedList = ({ userId }: LikedListProps) => {
  ...
  return (
    <div className="py-8 border-b border-gray-200">
      <p className="mb-4 text-xs font-semibold tracking-wider text-[#FF870D] uppercase">좋아요한 게시글</p>
      ...
      <ul className="space-y-3">
        {items.map((like) => (
          <li key={like.id}>
            <Link
              to={`/${like.feed.id}`}
              className="flex items-center justify-between p-3 border-0 rounded-lg hover:bg-gray-50"
            >
