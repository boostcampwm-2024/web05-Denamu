...
test("github oauth redirect params smoke @oauth-real", async ({ request }) => {
  const response = await request.get(`${baseURL}/api/oauth?type=github`, {
    maxRedirects: 0,
  });
  ...
  expect(authUrl.pathname).toContain("/login/oauth/authorize");

  const expectedRedirectUri = `${baseURL}/api/oauth/callback`;
  expect(authUrl.searchParams.get("redirect_uri")).toBe(expectedRedirectUri);
  expect(authUrl.searchParams.get("client_id")).toBeTruthy();
  expect(authUrl.searchParams.get("scope")).toContain("user:email");
  expect(authUrl.searchParams.get("scope")).toContain("read:user");
  expect(authUrl.searchParams.get("state")).toBeTruthy();
});
